...
import { ShoppingCart } from '../../src/shopping-cart/ShoppingCart.model';
import { AddToCartDto } from '../../src/shopping-cart/dto/AddToCart.dto';
...
describe('Shopping Cart Service', () => {
  ...
  it('should add cart items', async () => {
    const add = new AddToCartDto(mockedUser.username, 3);
    await shoppingCartService.addToCart(add);
    ...
    expect(cart.find((item) => item.partId === 3)).not.toBeUndefined();
